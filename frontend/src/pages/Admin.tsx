...
const Admin = () => {
    ...
    // === THIS IS THE CORRECTED AUTHENTICATION CHECK ===
    useEffect(() => {
        // 1. Wait for the authentication context to finish loading.
        if (loading) {
            return;
        }

        // 2. Once loading is false, safely check if the user is an admin.
        if (!user || user.role !== 'admin') {
            setAuthError('You are not authorized to access this page.');
            // Redirect any non-admin or logged-out user to the homepage.
            setTimeout(() => navigate('/'), 1500);
            return;
        }

        // 3. If the user is a confirmed admin, allow the component to render.
